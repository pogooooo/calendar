...
import * as S from "./BotanicalWeekCalendar.styles";
import CategoryFilter from "@/components/calendar/botanical/categoryFilter/CategoryFilter";
import AnimatedDateText from "@/components/calendar/botanical/animatedDateText/AnimatedDateText";
import TodoModal from "@/components/modal/todoModal/TodoModal";
import MoreModal from "@/components/modal/moreModal/MoreModal";
import TodoContextMenu from "@/components/calendar/botanical/contextMenu/TodoContextMenu";
...
type BotanicalWeekProps = WeekThemeProps & Omit<React.HTMLAttributes<HTMLDivElement>, 'contextMenu'>;

const BotanicalWeekCalendar = React.forwardRef<HTMLDivElement, BotanicalWeekProps>(
    ({
        ...
        return (
            <S.BotanicalCalendarWrapper as={Component} ref={ref} {...props}>
                <S.SliderWrapper>
                    <S.DateRangeDisplay>
                        <S.LeafAccent />
                        <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', display: 'flex', alignItems: 'center' }} onClick={handlePrevWeek}>
                            <ChevronLeft size={20} />
                        </button>
                        <AnimatedDateText text={dateRangeText} direction={direction} />
                        <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', display: 'flex', alignItems: 'center' }} onClick={handleNextWeek}>
                                            ...
                                            return (
                                                <S.StickerSlot key={`sticker-col-${idx}`} $isToday={isToday}>
                                                    {dayChallenges.map((challenge, cIdx) => (
                                                        ...
                                                        >
                                                            <BotanicalDynamicSticker isFilled={challenge.isDone} idx={cIdx + idx * 100} />
                                                        </div>
                                                    ))}
                                                </S.StickerSlot>
                                            )
...
BotanicalWeekCalendar.displayName = "BotanicalWeekCalendar";

export default BotanicalWeekCalendar;
